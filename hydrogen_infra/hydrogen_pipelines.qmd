...
The model distinguishes between infield pipelines and export pipelines, but documents them together because the same diameter, pressure, capacity, and material-cost logic applies. Installation is excluded from this page and is documented separately in [pipeline installation](../offshore_installation/pipeline_installation.qmd).
...
The current cost model covers material cost only. Installation, burial, connection, vessel time, simultaneous lay of parallel pipelines, and termination work are excluded here and treated in the installation section.

### Confidentiality Handling

The raw supplier quotes and numerical material-cost surface are confidential. They are used only to parameterise the model and are not reproduced in this encyclopedia page.

Any figure based on supplier prices is redacted before publication. @fig-pipeline-specific-cost-redacted shows only the qualitative shape of the relative material cost per unit transport capacity. It omits axes, tick labels, colour-bar values, absolute prices, and supplier quote points.

![Redacted relative material-cost surface for the pipeline options.](../figures/pipeline-specific-cost-redacted.svg){#fig-pipeline-specific-cost-redacted}
