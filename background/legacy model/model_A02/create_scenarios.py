...
Created on Fri Nov 21 09:50:13 2025

@author: VictorVerbist

Batch runner for multiple electricity / hydrogen scenarios.

Define scenarios here, run them, and collect KPIs in a table.
"""
...
out_path = 'results/results_big.csv'
...
def get_designs() -> List[Design]:
    ...
    num_turbines = [66, 133, 200, 267, 333, 400] # np.arange(50, 275, 25).astype(int).tolist()
    turbine_capacity_MW = [15.0]
    farm_area_km2 = np.arange( 50, 350, 25 )*1.0 # [200] # [50, 100, 150, 200, 250, 300]

    # hydrogen-only ranges (still defined here, in the same place)
    stack_overplant_factor = np.arange(10,21)/10 # [1.0, 1.2, 1.4, 1.6, 1.8 ,2] #  [1.0]#, 1.2, 1.4, 1.6, 1.8, 2.0]
    compressor_outlet_pressure_bar = np.arange(80,160,10)*1.0 #[150] # [150]#, 75, 100.0, 125, 150]
    export_pipeline_diameter_inch = np.arange(4,9,1)*1.0 # [8.0]
...
def run_all() -> pd.DataFrame:
    ...
    for design in designs:
        ...
        print(f"{s.scenario_id} | {design.carrier} | nt={design.num_turbines} | LCOE={s.results.total_LCOE:.2f}")
