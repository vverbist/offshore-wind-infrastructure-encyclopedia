...
TenneT's Dutch interface study used a nominal maximum of 630 A, equivalent to about 72 MVA at 66 kV. With reactive-power duty and a 0.9 power factor, it treated approximately 64–{{< var array-usable-string-rating >}} as usable active power per connection [@tennet2015JTubes]. The upper end of this range is used as the screening string rating on this page.
...
::: {.callout-note}
## Worked example: {{< var wind-turbine-rated-power >}} turbines

Four {{< var wind-turbine-rated-power >}} turbines place {{< var array-four-turbine-power >}} on a string. Five place {{< var array-five-turbine-power >}} on it and exceed the 64–{{< var array-usable-string-rating >}} screening range. A nominal {{< var array-reference-target-power >}} target therefore requires {{< var array-reference-turbine-count >}} turbines, giving {{< var array-reference-installed-power >}} installed and:

$$
n_\mathrm{string}=\left\lceil\frac{N_\mathrm{turb}}{n_\mathrm{turb/string}}\right\rceil.
$$

With {{< var array-turbines-per-string >}} turbines per string, the result is
{{< var array-reference-string-count >}} strings.
:::
...
![Required 66 kV radial strings for {{< var wind-turbine-rated-power >}} turbines and a {{< var array-usable-string-rating >}} usable string rating.](../figures/array-string-count.svg){#fig-array-string-count}

::: {.callout-warning}
## The worked array does not fit the reference platform unchanged

TenneT's {{< var array-reference-target-power >}} reference arrangement shows 24 dedicated wind-farm bays plus four universal bays [@tennet2024TwoGWProgram]. The {{< var array-reference-string-count >}}-string worked case therefore needs more terminations than the maximum 28 shown, even if every universal bay is assigned to a feeder. A consistent design must do one or more of the following:

- adopt a cable and platform interface rated for at least {{< var array-five-turbine-power >}} per string, so five {{< var wind-turbine-rated-power >}} turbines fit and only {{< var array-five-turbine-string-count >}} strings are required;
...
This interface constraint must be solved before the {{< var array-reference-string-count >}}-string cable count and standard {{< var array-reference-target-power >}} platform are combined in one cost case.
:::
...
| Usable string rating | 64–{{< var array-usable-string-rating >}} | Project-specific | TenneT screening range; final rating requires cable and interface checks |
| Turbine rating | {{< var wind-turbine-rated-power >}} | Model variable | Four turbines per base-case string |
...
| The {{< var array-reference-string-count >}}-string base case exceeds the reference platform bay count | Cable and platform designs are not yet a physically closed system |
